 ...
 * captured. Writes src/lib/starfield.ts. */
...
async function main() {
  ...
  const png = Buffer.from(await el.screenshot({ omitBackground: true, type: "png" }));
  await browser.close();
  ...
  await writeFile(
    file,
    `/**\n * LifeScript — baked starfield.\n *\n * A single pre-rendered PNG of the report's star layer. Replaces ~180 CSS\n * radial-gradient "stars" per page (which rendered as PDF shading patterns and\n * made the PDF slow to open in viewers). Visually identical; one image, reused\n * on every page. Regenerate with scripts/gen-starfield.ts if the star layer\n * design changes.\n */\nexport const STARFIELD_DATA_URI =\n  "${dataUri}";\n`,
    "utf8",
  );
